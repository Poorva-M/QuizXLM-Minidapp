...
impl QuizRewardContract {
    ...
    /// Returns reward amount in stroops (0 if wrong).
    ...
    /// Calculate total reward for a completed quiz session.
    /// Pure function — does not send any XLM.
    ...
    /// Called once per quiz session by the admin/backend.
    pub fn send_reward(
        env:             Env,
        admin:           Address,
        player:          Address,
...
mod test {
    ...
    fn test_calculate_reward_with_streak() {
        ...
        // 8 x 0.5 XLM + 5 XLM streak bonus = 9 XLM
